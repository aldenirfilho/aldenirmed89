# AldenirMed89 · Laranja Mecânica · Hepatite associada ao álcool

Módulo clínico estático, responsivo e sem dependências externas.

## Arquivos
- `index.html`: conteúdo, fluxo de decisão e referências.
- `styles.css`: identidade laranja/preto responsiva.
- `app.js`: calculadoras educacionais de Maddrey, ABIC e Lille.
- `module.manifest.json`: metadados, segurança e manutenção.

## Rota pública

`/aldenirmed89/01_Modulos_Clinicos/Hepatite_Alcool_Associada/`

## Segurança

Apoio educacional para profissionais de saúde. Validar diagnóstico, fórmulas, unidades, contraindicações, dose e protocolo institucional antes da prescrição. A página não armazena nem transmite os valores inseridos nas calculadoras.
